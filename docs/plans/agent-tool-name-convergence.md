...
注意：“当前 OV 是否已有对应接口”表示 OV 是否已有可以承载该语义的稳定接口，不表示插件暴露给 Agent 的工具名可以直接改成 `viking_xxx`。本 PR 不新增语义级 HTTP API，不改 CLI 命令，只收敛少量模型可见工具名；插件原有默认 scope、参数名、返回格式和 lifecycle 行为必须保持不变。
...
| OpenClaw | `memory_store` | `remember` | 无新增语义级接口；现有 REST `/api/v1/sessions/*` | 否 | 该工具语义是显式记忆写入：写 message、commit，并等待记忆提取完成；本 PR 不新增 HTTP remember API，也不改 OpenClaw 对外工具名和执行路径。 |
...
| Claude Code MCP | `store` | `remember` | 有：OV server `/mcp` 内部仍走 session 写入与 commit | 改工具名，不改执行语义 | 该工具是通用 MCP 客户端可见的显式记忆写入能力。OV server `/mcp` 已由 server identity 表明 provider，因此工具名使用能力名 `remember`，不带 `viking_` 前缀。 |
...
| Codex MCP | `openviking_store` | `remember` | 无新增语义级接口；现有 REST `/api/v1/sessions/*` | 改工具名，不改执行语义 | 该工具语义是显式记忆写入并同步等待提取结果；本 PR 只将 Codex 对外工具名收敛为 `remember`，内部仍按原逻辑 create session、add message、commit、cleanup。 |
...
- 不新增 `POST /api/v1/agent/remember`。
- 不改 OV CLI：`ov add-memory` 保持原命令名和原执行路径。
- Codex `openviking_store` 改名为 `remember`，但内部仍走原 session create/message/commit/cleanup 逻辑。
- Codex `openviking_forget` / `openviking_health` 改名为 `viking_forget` / `viking_health`，这两个语义和 OV 已有接口一致。
- OV server `/mcp` 的 `store` 改名为 `remember`，但内部仍走原 session 写入与 commit 逻辑。
- OpenClaw `memory_store` 保持工具名和执行路径不变。
