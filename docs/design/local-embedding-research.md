...
5. **当前 pyproject 没有本地推理依赖**：主干依赖中不存在 `sentence-transformers`、`transformers`、`torch`、`onnxruntime`、`llama-cpp-python`。无论选哪条路线，都要考虑 optional extra、wheel 体积和跨平台安装体验。
...
当前结论：**选定 `llama-cpp-python` 作为本地 embedding 实现方案**，默认模型采用 `bge-small-zh-v1.5-f16`。

| 方案 | 结论 | 主要原因 | 主要代价 |
|---|---|---|---|
| **llama-cpp-python** | **选定** | 与 QMD / OpenClaw 的 GGUF 路线一致，便于复用本地模型与缓存思路；可直接落地 `bge-small-zh-v1.5-f16` | 原生依赖安装和跨平台构建风险最高 |
| **sentence-transformers** | 未选 | Python 生态成熟，接口简单 | 依赖较重，和当前选定的 GGUF 模型路线不一致 |
| **ONNX Runtime** | 未选 | 推理性能和依赖体积相对平衡 | 需要切换到 ONNX 模型路线，不符合当前方案选择 |

### 候选 Embedding 模型

| 模型 | 维度 | 大小 | 多语言 | MTEB 表现 | 推荐场景 |
|---|---|---|---|---|---|
| `bge-small-zh-v1.5-f16` | 512 | ~130MB | 中文优化 | 良好 | 当前选定默认模型 |
| `embeddinggemma-300M` | 768 | ~300MB | 英文优化 | 良好 | QMD 默认参考 |
| `Qwen3-Embedding-0.6B` | 1024 | ~640MB | 100+ 语言 | 优秀 | 更大体积的多语言备选 |
| `multilingual-e5-small` | 384 | ~470MB | 多语言 | 良好 | 若后续放弃 GGUF 路线可再评估 |
...
| OpenClaw | node-llama-cpp 可选依赖导致安装问题 | 如果选择 GGUF + 本地原生推理路线，就必须把安装和运维风险当作一等设计问题 |

## 8. 风险与注意事项

1. **原生编译依赖的安装体验**（最高优先级）：OpenClaw 的 node-llama-cpp 实践证明，GGUF + C++ 原生推理库方案安装体验风险很高。Python 生态中的 `llama-cpp-python` 也存在类似风险。如果仍选择这条路线，就必须把安装文档、依赖隔离、失败提示和降级策略一起设计进去。
...
1. **第一阶段**：引入 `backend: "local"`，基于 `llama-cpp-python` 落地本地 GGUF embedding，并将默认模型设为 `bge-small-zh-v1.5-f16`。
2. **第二阶段**：补齐模型下载、缓存目录、显式 `model_path` 配置和依赖缺失提示；若 `llama-cpp-python` 或模型文件不可用，需要有清晰报错或回退策略。
3. **第三阶段**：扩展 embedder 接口，显式区分 `embed_query` / `embed_document`，或至少在 `embed(..., is_query=...)` 层面补齐语义，以适配 BGE 一类检索模型。
4. **第四阶段**：为 collection 持久化模型元数据（backend/model/dimension），模型变化时触发显式 rebuild，而不是依赖用户手工记忆。
5. **第五阶段**：补批量 embedding、下载进度、缓存目录和 benchmark，再根据真实体验决定是否保留该方案为默认路径。
