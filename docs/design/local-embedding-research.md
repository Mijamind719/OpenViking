...
2. **OpenClaw memory 本地方案**：OpenClaw memorySearch 支持 local provider，本地使用 GGUF embedding 模型推理
...
## 3. OpenClaw Memory 本地推理方案

### 3.1 官方默认路径

OpenClaw 官方的 LanceDB memory 插件默认仍以远程 OpenAI embedding 为主，本地 embedding 不是默认路径。这一点对 OpenViking 的意义在于：即便使用 LanceDB 作为向量存储，也不会自动获得“进程内本地 embedding”能力。

### 3.2 memorySearch 的本地 embedding 路线

OpenClaw 内置的 `memorySearch` 支持 `local` provider，本地通过 GGUF 模型和 `node-llama-cpp` 执行 embedding 推理。它体现了一个可参考的产品方向：

- 本地模型首次使用时自动下载
- embedding 能作为独立能力启用，不依赖远程 API Key
- 可以把本地 embedding 作为优先路径，其他 provider 作为 fallback

但它也暴露了一个非常明确的工程问题：一旦采用 `node-llama-cpp` 这类原生依赖，安装、升级和跨平台兼容性会显著恶化。OpenClaw 社区反馈的主要问题也集中在这里，而不是功能设计本身。
...
因此，OpenClaw 更像是为 OpenViking 提供了**产品方向上的支持**和**工程风险上的反例**。
