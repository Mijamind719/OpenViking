...
```bash
curl -fsSL https://raw.githubusercontent.com/volcengine/OpenViking/main/examples/openclaw-context-plugin/install.sh | bash
```

非交互模式：

```bash
curl -fsSL https://raw.githubusercontent.com/volcengine/OpenViking/main/examples/openclaw-context-plugin/install.sh | bash -s -y
```
...
看到 `memory-openviking: local server started` 表示成功（日志前缀为兼容性保留）。

### Step 4: 验证

```bash
openclaw status
# ContextEngine 行应显示：enabled (plugin openclaw-context-plugin)
```
...
```bash
git clone https://github.com/volcengine/OpenViking.git
cd OpenViking/examples/openclaw-context-plugin
npm install
...
openclaw config set plugins.entries.openclaw-context-plugin.config.baseUrl "http://your-server:1933"
openclaw config set plugins.entries.openclaw-context-plugin.config.apiKey "your-api-key"
...
```bash
# 在插件配置中指定
openclaw config set plugins.entries.openclaw-context-plugin.config.agentId "my-agent"
```
...
| 症状 | 原因 | 修复 |
|------|------|------|
| `port occupied` | 端口被其他进程占用 | 换端口：`openclaw config set plugins.entries.openclaw-context-plugin.config.port 1934` |
