...
cp examples/openclaw-context-plugin/skills/install-openviking-memory/SKILL.md \
...
copy examples\openclaw-context-plugin\skills\install-openviking-memory\SKILL.md ^
...
5. **Deploy plugin** — registers `openclaw-context-plugin` with OpenClaw
6. **Write env file** — generates `~/.openclaw/openviking.env`

> Non-interactive mode: `npx ./examples/openclaw-context-plugin/setup-helper -y`
...
# note: logger prefix remains `memory-openviking` for compatibility
```
...
```
npx ./examples/openclaw-context-plugin/setup-helper [options]
